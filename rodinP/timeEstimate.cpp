...
//const double max_feedrate[TimeEstimateCalculator::NUM_AXIS] = {600, 600, 40, 25};
//const double minimumfeedrate = 0.01;
//const double acceleration = 2500;
//const double max_acceleration[TimeEstimateCalculator::NUM_AXIS] = { 2500, 2500, 100, 10000 };
//const double max_xy_jerk = 25.0;
//const double max_z_jerk = 0.4;
//const double max_e_jerk = 5.0;
...
void TimeEstimateCalculator::updateInitValue()
{
	max_feedrate[0] = Profile::machineProfile.machine_max_feedrate_X.value;
	max_feedrate[1] = Profile::machineProfile.machine_max_feedrate_Y.value;
	max_feedrate[2] = Profile::machineProfile.machine_max_feedrate_Z.value;
	max_feedrate[3] = Profile::machineProfile.machine_max_feedrate_E.value;
	
	minimumfeedrate = Profile::machineProfile.machine_min_feedrate.value;
	
	acceleration = Profile::machineProfile.machine_acceleration.value;
	
	max_acceleration[0] = Profile::machineProfile.machine_max_acceleration_X.value;
	max_acceleration[1] = Profile::machineProfile.machine_max_acceleration_Y.value;
	max_acceleration[2] = Profile::machineProfile.machine_max_acceleration_Z.value;
	max_acceleration[3] = Profile::machineProfile.machine_max_acceleration_E.value;
	
	max_x_bump = Profile::machineProfile.machine_max_bump_X.value;
	max_y_bump = Profile::machineProfile.machine_max_bump_Y.value;
	max_z_bump = Profile::machineProfile.machine_max_bump_Z.value;
	max_e_bump = Profile::machineProfile.machine_max_bump_E.value;
}
...
void TimeEstimateCalculator::plan(Position newPos, double feedrate)
{
	...
	//vmax_junction 초기화..//
    double vmax_junction = max_x_bump / 2; 
	if (current_abs_feedrate[Y_AXIS] > max_y_bump / 2)
		vmax_junction = std::min(vmax_junction, max_y_bump / 2);
	if(current_abs_feedrate[Z_AXIS] > max_z_bump / 2)
        vmax_junction = std::min(vmax_junction, max_z_bump / 2);
	// hjkim delete 2021.10.21 extruder 의 max_junction 은 계산하지 않음 : 계산하면 지나치게 낮아짐.
    //if(current_abs_feedrate[E_AXIS] > max_e_bump / 2)
    //    vmax_junction = std::min(vmax_junction, max_e_bump / 2);
    ...
    {
		if (fabs(current_feedrate[X_AXIS] - previous_feedrate[X_AXIS]) > max_x_bump)
		{
			vmax_junction_factor = std::min(vmax_junction_factor, (max_x_bump / fabs(current_feedrate[X_AXIS] - previous_feedrate[X_AXIS])));
        }
		if (fabs(current_feedrate[Y_AXIS] - previous_feedrate[Y_AXIS]) > max_y_bump)
		{
			vmax_junction_factor = std::min(vmax_junction_factor, (max_y_bump / fabs(current_feedrate[Y_AXIS] - previous_feedrate[Y_AXIS])));
		}
        if(fabs(current_feedrate[Z_AXIS] - previous_feedrate[Z_AXIS]) > max_z_bump)
		{
			vmax_junction_factor = std::min(vmax_junction_factor, (max_z_bump / fabs(current_feedrate[Z_AXIS] - previous_feedrate[Z_AXIS])));
        } 
        if(fabs(current_feedrate[E_AXIS] - previous_feedrate[E_AXIS]) > max_e_bump)
		{
			vmax_junction_factor = std::min(vmax_junction_factor, (max_e_bump / fabs(current_feedrate[E_AXIS] - previous_feedrate[E_AXIS])));
        } 
...
double TimeEstimateCalculator::calculate()
{
    ...
    double totalTime = 0;
	// hjkim 임시 수정 가속, 감속, 등속의 시간을 알기 위하여 따로 계산 (2021.10.21)
	double acceleration_time = 0;
	double plateau_time = 0;
	double decelerate_time = 0;
	double sum_plateau_distance = 0;
	if (blocks.size() == 7974)  
	{
		for (unsigned int n = 0; n < blocks.size(); n++)
		{
			std::cout << blocks[n].distance <<":"<<blocks[n].accelerate_until<<":" << blocks[n].decelerate_after << ":"<< blocks[n].nominal_feedrate << ":"<< blocks[n].acceleration << ":" << acceleration_time_from_distance(blocks[n].initial_feedrate, blocks[n].accelerate_until, blocks[n].acceleration) << ":"<< acceleration_time_from_distance(blocks[n].final_feedrate, (blocks[n].distance - blocks[n].decelerate_after), blocks[n].acceleration) << std::endl;
		}
	}
    for(unsigned int n=0; n<blocks.size(); n++)
    {
        double plateau_distance = blocks[n].decelerate_after - blocks[n].accelerate_until;
		sum_plateau_distance += plateau_distance;
		acceleration_time += acceleration_time_from_distance(blocks[n].initial_feedrate, blocks[n].accelerate_until, blocks[n].acceleration);
		plateau_time += plateau_distance / blocks[n].nominal_feedrate;
		decelerate_time += acceleration_time_from_distance(blocks[n].final_feedrate, (blocks[n].distance - blocks[n].decelerate_after), blocks[n].acceleration);
    } 
	totalTime = acceleration_time + plateau_time + decelerate_time;
    return totalTime;
}
...
void TimeEstimateCalculator::clear()
{
	previous_feedrate = Position();
