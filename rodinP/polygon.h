...
//     }
    ...
    Point min() const
    {
        ...
        {
            ret.X = std::min(ret.X, (*polygon)[i].X);
            ret.Y = std::min(ret.Y, (*polygon)[i].Y);
        }
        return ret;
    }
	...
	Point max() const
    {
		...
		for(int i = 0; i < (*polygon).size(); ++i)
		{
			ret.X = std::max(ret.X, (*polygon)[i].X);
			ret.Y = std::max(ret.Y, (*polygon)[i].Y);
		}
        return ret;
    }
	...
	Point_D min_d() const
	{
		Point_D ret = Point_D(POINT_MAX, POINT_MAX);
		for (int i = 0; i < (*polygon).size(); ++i)
		{
			ret.X = std::min(ret.X, double((*polygon)[i].X));
			ret.Y = std::min(ret.Y, double((*polygon)[i].Y));
		}
		return ret;
	}
	...
	Point_D max_d() const
	{
		Point_D ret = Point_D(POINT_MIN, POINT_MIN);
		for (int i = 0; i < (*polygon).size(); ++i)
		{
			ret.X = std::max(ret.X, double((*polygon)[i].X));
			ret.Y = std::max(ret.Y, double((*polygon)[i].Y));
		}
		return ret;
	}
...
public:
	...
	Polygons offset_line(int distance, ClipperLib::JoinType joinType = ClipperLib::jtMiter) const
	{
		Polygons ret;
		double miterLimit = 1.2;
		ClipperLib::ClipperOffset clipper(miterLimit, 10.0);
...
public:
	...
	bool isDegenerate(Point& last, Point& now, Point& next)
	{
...
//          for (PolygonRef poly_rem : to_be_removed)
            ...
            {
				PolygonRef poly_rem = to_be_removed[i];
...
//        for(const ClipperLib::Path& polygon : polygons)
		for(int i = 0; i < polygons.size(); ++i)
        {
            ...
            {
                ret.X = std::min(ret.X, polygons[i][j].X);
                ret.Y = std::min(ret.Y, polygons[i][j].Y);
            }
        }
        return ret;
    }
    ...
    Point max() const
    {
		...
		for(int i = 0; i < polygons.size(); ++i)
		{
			...
			for(int j = 0; j < polygons[i].size(); ++j)
            {
				ret.X = std::max(ret.X, polygons[i][j].X);
				ret.Y = std::max(ret.Y, polygons[i][j].Y);
            }
        }
        return ret;
    }
	...
	Point_D min_d() const
	{
		...
		for (int i = 0; i < polygons.size(); ++i)
		{
			...
			for (int j = 0; j < polygons[i].size(); ++j)
			{
				ret.X = std::min(ret.X, double(polygons[i][j].X));
				ret.Y = std::min(ret.Y, double(polygons[i][j].Y));
			}
		}
		return ret;
	}
	...
	Point_D max_d() const
	{
		...
		{	//for(Point p : polygon)
			for (int j = 0; j < polygons[i].size(); ++j)
			{
				ret.X = std::max(ret.X, double(polygons[i][j].X));
				ret.Y = std::max(ret.Y, double(polygons[i][j].Y));
			}
		}
		return ret;
	}
